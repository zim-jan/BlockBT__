...
export const InspectorPanel: React.FC = () => {
  ...
  return (
    <aside className="flex-none w-96 h-full bg-[#131722] border-l border-white/10 shadow-2xl flex flex-col text-slate-100 relative">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4.5 border-b border-white/10 bg-[#1c2130]/80 flex-none">
...
const DataNodeInspector: React.FC<{ data: DataNodeData; onChange: (k: string, v: any) => void }> = ({ data, onChange }) => {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Data Source</label>
        <select
      ...
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Ticker Symbol(s)</label>
        <input
          type="text"
      ...
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Timeframe</label>
        <select
      ...
      <div className="flex items-center gap-2.5 pt-4 mt-2 border-t border-white/10">
        ...
        />
        <label htmlFor="pit-enforce" className="text-xs text-slate-300 cursor-pointer font-medium select-none">
...
const IndicatorNodeInspector: React.FC<{ data: IndicatorNodeData; onChange: (k: string, v: any) => void }> = ({ data, onChange }) => {
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Indicator Type</label>
        <select
      ...
      {data.indicatorType === 'sma_crossover' && (
        <div className="grid grid-cols-2 gap-3.5">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Fast Moving Average</label>
            <input
              type="number"
              min={2}
              max={200}
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Slow Moving Average</label>
            <input
              type="number"
              min={5}
              max={500}
      ...
      {data.indicatorType === 'macd' && (
        <div className="grid grid-cols-3 gap-2.5">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Fast EMA</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Slow EMA</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Signal Line</label>
            <input
              type="number"
      ...
      {data.indicatorType === 'rsi' && (
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">RSI Period (Window)</label>
            <input
              type="number"
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Oversold (Lower Limit)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Overbought (Upper Limit)</label>
              <input
                type="number"
      ...
      {data.indicatorType === 'custom' && (
        <div>
          <label className="block text-xs font-medium text-slate-300 mb-1.5">Indicator Code (Numba JIT Sandbox)</label>
          <textarea
            rows={8}
      ...
      {isDynamic && currentRegistryInd?.params && (
        <div className="space-y-4 pt-4 mt-2 border-t border-white/10">
          <span className="text-xs font-semibold text-purple-400 uppercase tracking-wider block mb-1">Introspection Parameters</span>
          {currentRegistryInd.params.map((p: any) => (
            <div key={p.name}>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">{p.name}</label>
              <input
...
const SignalNodeInspector: React.FC<{ data: SignalNodeData; onChange: (k: string, v: any) => void }> = ({ data, onChange }) => (
  <div className="space-y-5">
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">Logic Operator Type</label>
      <select
...
const PortfolioNodeInspector: React.FC<{ data: PortfolioNodeData; onChange: (k: string, v: any) => void }> = ({ data, onChange }) => (
  <div className="space-y-5">
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">Initial Capital ($)</label>
      <input
        type="number"
    ...
    <div className="grid grid-cols-2 gap-3.5">
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Fees (%)</label>
        <input
          type="number"
          step="0.01"
    ...
    <div className="pt-4 mt-2 border-t border-white/10 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3.5">
        <div>
          <label className="block text-xs font-medium text-slate-300 mb-1.5">Position Size</label>
          <input
            type="number"
            value={data.size ?? ''}
        ...
        <div>
          <label className="block text-xs font-medium text-slate-300 mb-1.5">Size Type</label>
          <select
...
const OptimizerNodeInspector: React.FC<{
  ...
  return (
    <div className="space-y-5">
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Optimization Target Metric</label>
        <select
      ...
      <div>
        <label className="block text-xs font-medium text-slate-300 mb-1.5">Optuna Trials (n_trials)</label>
        <input
          type="number"
          min={5}
          max={500}
      ...
      <div className="pt-4 mt-2 border-t border-white/10 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[11px] font-medium bg-[#1c2130] hover:bg-[#262c3e] border border-pink-500/30 text-pink-300 px-2.5 py-1 rounded transition-colors"
          >
        ...
        <div className="space-y-3">
          {Object.entries(bounds).map(([key, bound]) => (
            <div key={key} className="bg-[#0b0d14]/80 p-4 rounded-xl border border-white/10 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#131722] border border-white/10 rounded px-2.5 py-1.5 text-xs text-slate-100 font-mono"
                  />
                </div>
                <div>
      ...
      {data.bestParameters && (
        <div className="p-4 bg-pink-950/30 border border-pink-500/30 rounded-xl space-y-3 mt-5">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-pink-300">Best Result:</span>
            <button
              onClick={handleApplyToIndicator}
              className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold bg-emerald-600 hover:bg-emerald-500 text-white rounded transition-colors"
            >
          ...
          <div className="text-xs font-mono text-slate-300 space-y-1.5">
            ...
            {data.bestValue != null && (
              <div className="pt-2 border-t border-pink-500/20 text-emerald-400 font-medium">Score: {data.bestValue.toFixed(4)}</div>
            )}
          </div>
        </div>
      )}
...
const WfoNodeInspector: React.FC<{ data: WfoNodeData; onChange: (k: string, v: any) => void }> = ({ data, onChange }) => (
  <div className="space-y-5">
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">Walk-Forward Mode</label>
      <select
    ...
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">In-Sample Window Size</label>
      <input
        type="text"
    ...
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">Step Size</label>
      <input
        type="text"
    ...
    <div>
      <label className="block text-xs font-medium text-slate-300 mb-1.5">Evaluation Metric</label>
      <select
