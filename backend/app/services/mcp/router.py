...
def get_domain_context(domain: str) -> str:
    ...
    contexts = {
        "data_connector": (
            ...
            "Guidelines:\n"
            "- Must extend base class `base.BaseDataConnector` (set CONNECTOR_KEY).\n"
            "- Implement sync `_download()`; public entrypoint is inherited `fetch()`.\n"
            "- Data is cached locally as Parquet files under `settings.PARQUET_DIR` "
            "(path components are sanitized — audyt 2026-07-17).\n"
        ...
        "core_engine": (
            ...
            "- Faza 15 (walk-forward): `WalkForwardOptimizer` (`optimizer.py`) is a REAL WFO (stub "
            "replaced). `split_windows(index, window_size, step_size, mode)` yields half-open IS "
            "`[is_start, is_end)` / OOS `[is_end, oos_end)` windows (`oos_start == is_end` -> no "
            "look-ahead by construction); modes: `rolling` (fixed IS length, shifts by step) and "
            "`anchored` (IS grows from data start); `step_size` is both OOS length and stride "
            "(adjacent, non-overlapping OOS segments). Per window: optional in-sample optimization "
            "reusing `OptunaOptimizer` (`param_bounds`/`n_trials`/`metric`), then OOS backtest on "
            "best params; a failed window yields 0.0 metrics + `error` (does not abort the WFO). "
            "Aggregation: geometric compound Total Return + mean Sharpe over OOS windows, NaN/inf "
            "-> 0.0 (`_finite_or_zero`). Result carries `best_params`/`best_value`/`trials` "
            "(JobService contract; window reports land in `trials_data.trials`, read by the "
            "frontend `WfoNode`). See ADR-0006."
        ),
        "api_layer": (
            "Domain: API Layer\n"
            ...
            "- Faza 15 (walk-forward): `POST /api/optimizer/wfo` — `WalkForwardRequest` extended "
            "ADDITIVELY with optional `mode` (`Literal['rolling','anchored']`, default `rolling`), "
            "`param_bounds` (`dict[str, ParameterBounds]`, enables per-window in-sample Optuna), "
            "`n_trials` (1..500, default 15) and `metric` (default `Total Return [%]`). Defaults "
            "preserve the old contract (frontend `WfoNode` sends only `window_size`/`step_size`). "
            "Background worker `run_walk_forward` (runner) has matching optional kwargs; window "
            "reports are stored in `OptimizationJob.trials_data['trials']`."
        ),
        "frontend": (
