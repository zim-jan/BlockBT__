...
- **Agregacja wyników**: raport zawiera metryki per okno (granice IS/OOS, `best_params` bez kluczy infrastrukturalnych, `oos_metrics` z metryką celu) oraz łączne metryki OOS — zwrot całkowity składany geometrycznie z okien **udanych** i Sharpe Ratio liczony ze sklejonych zwrotów OOS (`equity_curve` okien; fallback: średnia per okno); wartości NaN/inf są sprowadzane do `0.0` (konwencja silnika). Awaria optymalizacji IS lub backtestu OOS pojedynczego okna nie przerywa całego WFO (okno dostaje metryki `0.0` i pole `error` i jest wykluczane z agregatów); awaria wszystkich okien kończy job statusem `FAILED`. `best_value` to wartość metryki celu w najlepszym oknie OOS (analogicznie do najlepszego triala Optuny).
